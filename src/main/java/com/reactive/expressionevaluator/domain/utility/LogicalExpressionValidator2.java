...
public class LogicalExpressionValidator2 {
    ...
    private static <T> Function<T, Boolean> buildEvaluatorAND(String expression) {
        ...
        String[] clauses = expression.split("\\s+AND\\s+");

        Predicate<T> predicate = t -> {
            for (String clause : clauses) {
                //String[] conditions = clause.split("\\s+OR\\s+");
                String[] conditionsOR = clause.split("\\|\\|");
                String[] conditionsAND = clause.split("&&");
                if(conditionsOR.length > 1){
                    ...
                    return evaluateCondition(clause, t);
                }
    ...
    private static <T> Function<T, Boolean> buildEvaluatorOR(String expression) {
        ...
        Predicate<T> predicate = t -> {
            for (String clause : clauses) {
                String[] conditions = clause.split("\\s+&&\\s+");
                boolean clauseResult = true;
...
//customer.setFirstName("JOHN");
  ...
  //      address.setCity("Washington");
   ...
   //     customer.setAddress(address);
       ...
       // String expression = "(firstName == \"JOHN\" AND salary > 100) AND (address != null AND address.city == \"Washington\") AND (firstName == \"X\" && salary > 100)";
